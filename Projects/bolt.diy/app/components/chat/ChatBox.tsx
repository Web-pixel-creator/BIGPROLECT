...
export const ChatBox: React.FC<ChatBoxProps> = (props) => {
  ...
  const isBriefMode = props.inputMode === 'brief' && !props.chatStarted;
  const handleBriefSubmit = props.onBriefSubmit ?? (() => undefined);
  ...
  return (
    ...
    >
      ...
      {!props.chatStarted && props.inputMode && props.setInputMode && (
              ...
              <div className="flex gap-1 items-center">
                ...
                <CustomModelSelector
                  ...
                  onModelChange={(modelId) => {
                    ...
                    if (
                      modelLower.startsWith('iic/') ||
                      modelLower.startsWith('qwen/') ||
                      modelLower.startsWith('deepseek-ai/')
                    ) {
                      providerName = 'ModelScope';
                    } else if (modelLower.startsWith('moonshotai/')) {
                      // Kimi K2.5 and other Moonshot models via OpenRouter
                    ...
                    console.log('[CustomModelSelector] Selected model:', modelId, 'Provider:', providerName);
                    console.log('[CustomModelSelector] Available providers:', props.providerList.map(p => p.name));

                    // Try to find provider in active list first, fallback to PROVIDER_LIST
                    let providerInfo = props.providerList.find((p) => p.name === providerName);

                    if (!providerInfo) {
                      providerInfo = (PROVIDER_LIST as ProviderInfo[]).find((p) => p.name === providerName);
                      console.log('[CustomModelSelector] Provider not in active list, using fallback:', providerName);
                    }

                    if (providerInfo && props.setProvider) {
                      console.log('[CustomModelSelector] Setting provider to:', providerInfo.name);
                      props.setProvider(providerInfo);
                    } else {
                      console.error('[CustomModelSelector] Could not find provider:', providerName);
                    }
